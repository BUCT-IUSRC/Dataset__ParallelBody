...
class MineDataset(DatasetTemplate):
    ...
    def get_radar(self, idx):
        lidar_file = self.root_split_path / 'arbe' / ('%s.bin' % idx)
        assert lidar_file.exists()
        #修改1  点数变为5
        points_arbe = np.fromfile(str(lidar_file), dtype=np.float32).reshape(-1, 4)
        num_point = points_arbe.shape[0]
        points_arbe_hom = np.hstack((points_arbe[:,0:3],np.ones((num_point,1))))
        ARB2V = np.array([-0.013970759076831379,0.9985046529494882,0.05285145153199537, 2.2198703343289825,-0.9998950756585999, -0.014153495799834579, 0.0030848387844398294, 0.02787126097813525, 0.003828258677135485, -0.05280280858880308, 0.9985976205863071, -1.2864166043457155]).reshape([3, 4])
    ...
    def __getitem__(self, index):
        ...
        points = self.get_lidar(sample_idx)[:,0:4]
        radar_points = self.get_radar(sample_idx)[:,0:4]
        calib = self.get_calib(sample_idx)
        ...
        if 'annos' in info:
            ...
            #fangadd
            hflipped = False
            targets = self.build_targets(gt_boxes_lidar, hflipped)
    ...
    def build_targets(self, labels, hflipped):
        ...
        num_objects = min(len(labels), 50)
        # 修改
        hm_l, hm_w = (40, 40)

        hm_main_center = np.zeros((1,int(hm_l), int(hm_w)), dtype=np.float32)
        cen_offset = np.zeros((50, 2), dtype=np.float32)
        direction = np.zeros((50, 2), dtype=np.float32)
        z_coor = np.zeros((50, 1), dtype=np.float32)
        dimension = np.zeros((50, 3), dtype=np.float32)
        indices_center = np.zeros((50), dtype=np.int64)
        obj_mask = np.zeros((50), dtype=np.uint8)

        for k in range(num_objects):
            x, y, z, l, w, h, yaw = labels[k]
            cls_id = 0
            # Invert yaw angle
            yaw = -yaw
            ...
            # 修改 69.12->51.2
            center_x = (x - minX) / 51.2 * hm_w  # x --> y (invert to 2D image space)
            center_y = (y - minY) / 51.2 * hm_l  # y --> x
...
'''
data_path = os.path.join(ROOT_DIR, 'data', 'mine', 'lidar'),
save_path = os.path.join(ROOT_DIR, 'data', 'mine', 'lidar')'''