...
#
#         self.conv_up_level1 = nn.Conv2d(128, 256, kernel_size=1, stride=1, padding=0)
#         self.conv_up_level2 = nn.Conv2d(320, 128, kernel_size=1, stride=1, padding=0)
...
#             for fpn_idx, fdn_input in enumerate([up_level2,]):
...
#         # print("x",x.size()) #b,32,10
#         # print("y",y.size()) #b,32,10
#         x = self.linear(x).permute(0, 2, 1) # b,64,32
#         y = self.linear(y).permute(0, 2, 1) #b,64,32
#         # print("x",x.size()) #b,64,32
#         # print("y",y.size()) #b,64,32
#         x_q = self.q_conv(x)  #b,16,32
#         y_k = self.k_conv(y).permute(0,2,1)# b, 32,16
#         y_v = self.v_conv(y)  #b,16,32
#         print(y_k.size())
#         print(x_q.size())
#         # energy = torch.bmm(y_k,x_q) # b, n, n  # b,c,c
#         # print("my version-----------")
#         energy = torch.matmul(x_q,y_k) # b, n, n  # b,c,c # b1,
#         attention = self.softmax(energy) # b,16,16
#         attention = attention / (1e-9 + attention.sum(dim=1, keepdims=True))
#         # y_r = torch.bmm(y_v,attention).permute(1, 2, 0) # b, c, n
#         y_r = torch.bmm(attention, y_v) # b, 16,32
...
class PillarVFE(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range):
        ...
        self.AFDet = PoseResNet(block_class, layers, heads, head_conv=head_conv,output_shape=dense_shape, voxel_generator=voxel_generator)
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        for batch in range(batch_size):
            ...
            for j in range(len(ys[batch])):
                if j == 0:
                    y_008 = torch.arange(int(ys_008[batch][j]+0.5)-24, int(ys_008[batch][j]+0.5)+24)
                else:
                    y_buff_008 = torch.arange(int(ys_008[batch][j]+0.5)-24, int(ys_008[batch][j]+0.5)+24)
...
# for i in range(batch_size):
        #     if i == 0:
        #         detection_0 = detections[i]
        #         kitti_dets_0 = convert_det_to_real_values(detection_0,configs.num_classes)
        #     if i == 1:
        #         detection_1 = detections[i]
        #         kitti_dets_1 = convert_det_to_real_values(detection_1,configs.num_classes)
        #         kitti_dets = np.array([kitti_dets_0, kitti_dets_1])

        # # voxel->point
        # num_obj = kitti_dets.shape[0]
        # for batch in range(batch_size):
        #     if batch == 0:
        #         lidar_batch_mask = batch_dict['lidar_voxel_coords'][:, 0] == batch
        #         lidar_voxel_point = batch_dict['lidar_voxels'][lidar_batch_mask, :]
        #         lidar_voxel_point = lidar_voxel_point.view(lidar_voxel_point.shape[0]*lidar_voxel_point.shape[1], -1)
        #         nonzero_mask = lidar_voxel_point[:, 0] + lidar_voxel_point[:, 1] + lidar_voxel_point[:, 2] != torch.zeros(1, device='cuda:0')
        #         lidar_voxel_point = lidar_voxel_point[nonzero_mask]

        #         lidar_point_indices = roiaware_pool3d_utils.points_in_boxes_cpu(
        #             lidar_voxel_point[:, 0:3], torch.from_numpy(kitti_dets[batch][:, 1:8])
        #         ).numpy()  # (nboxes, npoints)
        #         num_obj = kitti_dets[batch].shape[0]
        #         for i in range(num_obj):
        #             if i == 0:
        #                 lidar_gt_points = lidar_voxel_point[lidar_point_indices[i] > 0]
        #             else:
        #                 lidar_gt_points = torch.cat((lidar_gt_points, lidar_voxel_point[lidar_point_indices[i]]))
        #         lidar_voxel_output_008 = self.voxel_generator_008.generate(lidar_gt_points)
        #         if isinstance(lidar_voxel_output_008, dict):
        ...
        #         lidar_voxel_output_008 = self.voxel_generator_008.generate(lidar_gt_points)
        #         if isinstance(lidar_voxel_output_008, dict):
        #             lidar_voxels_008 = torch.cat((lidar_voxels_008, lidar_voxel_output_008['voxels']))
        #             lidar_coordinates_008 = torch.cat((lidar_coordinates_008, lidar_voxel_output_008['coordinates']))
        #             lidar_num_points_008 = torch.cat((lidar_num_points_008, lidar_voxel_output_008['num_points_per_voxel']))
        #         else:
        #             lidar_voxels_008_, lidar_coordinates_008_, lidar_num_points_008_ = lidar_voxel_output_008
        #             lidar_voxels_008 = torch.cat((lidar_voxels_008, lidar_voxels_008_))
        #             lidar_coordinates_008 = torch.cat((lidar_coordinates_008, lidar_coordinates_008_))
        #             lidar_num_points_008 = torch.cat((lidar_num_points_008, lidar_num_points_008_))

        # for batch in range(batch_size):
        #     if batch == 0:
        #         radar_batch_mask = batch_dict['radar_voxel_coords'][:, 0] == batch
        #         radar_voxel_point = batch_dict['radar_voxels'][radar_batch_mask, :]
        #         radar_voxel_point = radar_voxel_point.view(radar_voxel_point.shape[0]*radar_voxel_point.shape[1], -1)
        #         radar_point_indices = roiaware_pool3d_utils.points_in_boxes_cpu(
        #             radar_voxel_point[:, 0:3], torch.from_numpy(kitti_dets[batch][:, 1:8])
        #         ).numpy()  # (nboxes, npoints)
        #         num_obj = kitti_dets[batch].shape[0]
        #         for i in range(num_obj):
        #             if i == 0:
        #                 radar_gt_points = radar_voxel_point[radar_point_indices[i] > 0]
        #             else:
        #                 radar_gt_points = torch.cat((radar_gt_points, radar_voxel_point[radar_point_indices[i]]))
        #         radar_voxel_output_008 = self.voxel_generator_008.generate(radar_gt_points)
        #         if isinstance(radar_voxel_output_008, dict):
        ...
        #         radar_voxel_output_008 = self.voxel_generator_008.generate(radar_gt_points)
        #         if isinstance(radar_voxel_output_008, dict):
        #             radar_voxels_008 = torch.cat((radar_voxels_008, radar_voxel_output_008['voxels']))
        #             radar_coordinates_008 = torch.cat((radar_coordinates_008, radar_voxel_output_008['coordinates']))
        #             radar_num_points_008 = torch.cat((radar_num_points_008, radar_voxel_output_008['num_points_per_voxel']))
        #         else:
        #             radar_voxels_008_, radar_coordinates_008_, radar_num_points_008_ = radar_voxel_output_008
        #             radar_voxels_008 = torch.cat((radar_voxels_008, radar_voxels_008_))
        #             radar_coordinates_008 = torch.cat((radar_coordinates_008, radar_coordinates_008_))
        #             radar_num_points_008 = torch.cat((radar_num_points_008, radar_num_points_008_))



        # lidar_point_indices = roiaware_pool3d_utils.points_in_boxes_cpu(
        #     lidar_points[:, 0:3], torch.from_numpy(kitti_dets[:, 1:8])
        # ).numpy()  # (nboxes, npoints)
