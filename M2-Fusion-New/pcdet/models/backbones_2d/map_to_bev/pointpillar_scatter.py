...
class PointPillarScatter(nn.Module):
    def __init__(self, model_cfg, grid_size, **kwargs):
        ...
        self.adap_max_pool = nn.MaxPool2d(2,stride=2)
        # self.adap_max_pool = nn.AdaptiveMaxPool2d(480)
        # self.adap_max_pool = nn.AdaptiveMaxPool2d(440)

        self.linear = nn.Linear(256, 64, bias=True)
    # @profile
    def forward(self, batch_dict, **kwargs):
        ...
        for radar_cen_batch_idx in range(radar_cen_batch_size):
            ...
            radar_cen_indices = radar_cen_this_coords[:, 1] + radar_cen_this_coords[:, 2] * 320 + radar_cen_this_coords[:, 3]
        ...
        radar_cen_batch_spatial_features = radar_cen_batch_spatial_features.view(radar_cen_batch_size, self.num_bev_features * self.nz, 320, 320)
        ...
        for lidar_cen_batch_idx in range(lidar_cen_batch_size):
            ...
            lidar_cen_indices = lidar_cen_this_coords[:, 1] + lidar_cen_this_coords[:, 2] * 320 + lidar_cen_this_coords[:, 3]
        ...
        lidar_cen_batch_spatial_features = lidar_cen_batch_spatial_features.view(lidar_cen_batch_size, self.num_bev_features * self.nz, 320, 320)
        ...
        ########################################### cat操作 ##############################################
        # lidar_batch_spatial_features = batch_dict['afdet_spatial_features']
        # print("###############################")
        # print(lidar_batch_spatial_features.shape)
        # print(lidar_cen_batch_spatial_features.shape)
        ...
        # cat_batch_spatial_features = self.linear(cat_batch_spatial_features.permute(0,2,3,1))

        # cat_batch_spatial_features = cat_batch_spatial_features.permute(0,3,1,2)
